...
void hisi_uncore_pmu_write_evtype(struct hisi_pmu *phisi_pmu,
						int idx, u32 val)
{
	/* Select event based on Hardware counter Module */
	phisi_pmu->set_evtype(phisi_pmu, idx, val);
}

int hisi_pmu_get_event_idx(struct hisi_pmu *phisi_pmu)
{
	int event_idx = -1;

	/* Get the available hardware event counter index */
	event_idx = phisi_pmu->get_event_idx(phisi_pmu);

	return event_idx;
}

void hisi_pmu_clear_event_idx(struct hisi_pmu *phisi_pmu, int idx)
{
	/* Release the hardware event counter index */
	phisi_pmu->clear_event_idx(phisi_pmu, idx);
}
...
u64 hisi_uncore_pmu_event_update(struct perf_event *event,
	 ...
	 */
	new_raw_count = phisi_pmu->event_update(event, hwc, idx);
...
void hisi_pmu_enable_counter(struct hisi_pmu *phisi_pmu, int idx)
{
	/* Enable the hardware event counting */
	phisi_pmu->enable_counter(phisi_pmu, idx);
}

void hisi_pmu_disable_counter(struct hisi_pmu *phisi_pmu, int idx)
{
	/* Disable the hardware event counting */
	phisi_pmu->disable_counter(phisi_pmu, idx);
}
...
void hisi_uncore_pmu_enable_event(struct perf_event *event)
{
	 ...
	 * Disable counter
	 */
	hisi_pmu_disable_counter(phisi_pmu, GET_CNTR_IDX(hwc));

	/*
	 * Set event (if destined for Hisilicon SoC counters).
	 */
	hisi_uncore_pmu_write_evtype(phisi_pmu, GET_CNTR_IDX(hwc),
							hwc->config_base);

	/*
	 * Enable counter
	 */
	hisi_pmu_enable_counter(phisi_pmu, GET_CNTR_IDX(hwc));
}

int hisi_pmu_write_counter(struct hisi_pmu *phisi_pmu, int idx,
							u32 value)
{
	int ret = 0;

	/* Write to the hardware event counter */
	ret = phisi_pmu->write_counter(phisi_pmu, idx, value);
...
void hisi_pmu_event_set_period(struct perf_event *event)
{
	...
	hisi_pmu_write_counter(phisi_pmu, GET_CNTR_IDX(hwc), val);
}
						...
						int pmu_flags)
{
	...
	if (flags & PERF_EF_RELOAD) {
		...
		hisi_pmu_write_counter(phisi_pmu, GET_CNTR_IDX(hwc),
							(u32)prev_raw_count);
	}
						...
						int flags)
{
	...
	hisi_uncore_pmu_event_update(event, phisi_pmu, hwc, GET_CNTR_IDX(hwc));
...
int hisi_uncore_pmu_add(struct perf_event *event, int flags)
{
	...
	idx = hisi_pmu_get_event_idx(phisi_pmu);
	if (idx < 0)
		return -EAGAIN;
...
void hisi_uncore_pmu_del(struct perf_event *event, int flags)
{
	...
	hisi_pmu_clear_event_idx(phisi_pmu, GET_CNTR_IDX(hwc));
	perf_event_update_userpage(event);
...
struct hisi_pmu *hisi_pmu_alloc(struct platform_device *pdev)
{
	...
	phisi_pmu = devm_kzalloc(&pdev->dev, sizeof(*phisi_pmu), GFP_KERNEL);
	if (!phisi_pmu)
