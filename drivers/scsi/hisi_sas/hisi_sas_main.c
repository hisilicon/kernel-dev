...
static int hisi_sas_slot_index_alloc(struct hisi_hba *hisi_hba, int *slot_idx)
{
	...
	if (index >= hisi_hba->slot_index_count) {
		index = find_next_zero_bit(bitmap, hisi_hba->slot_index_count, 0);
		if (index >= hisi_hba->slot_index_count)
...
static void hisi_sas_send_ata_reset_each_phy(struct hisi_hba *hisi_hba,
		struct asd_sas_port *sas_port, struct domain_device *device)
{
	...
	list_for_each_entry(sas_phy, &sas_port->phy_list,
			port_phy_el) {
		if (!(state & BIT(sas_phy->id)))
			continue;
			     ...
			     int abort_flag, int tag)
{
	...
	task->task_done = hisi_sas_task_done;
	task->slow_task->timer.function = hisi_sas_tmf_timedout;
	task->slow_task->timer.expires = jiffies + INTER_ABORT_TIMEOUT*HZ;
	add_timer(&task->slow_task->timer);
