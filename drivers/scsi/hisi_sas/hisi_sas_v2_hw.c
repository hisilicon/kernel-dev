...
void print_prd_sge_v2_hw(struct hisi_sas_slot *slot)
{
	...
	int i;
	pr_err("%s scatter=%pS slot->n_elem=%lld n_elem=%d\n", __func__, scatter, slot->n_elem, task->num_scatter);
	for_each_sg(scatter, sg, n_elem, i) {
		struct hisi_sas_sge *entry = &sge_page->sge[i];
		void *virt = sg_virt(sg);
			  ...
			  struct hisi_sas_slot *slot)
{
	...
	pr_err("%s task=%pS lldd_task=%pS has_data=%d\n", __func__, task, task->lldd_task, has_data);
