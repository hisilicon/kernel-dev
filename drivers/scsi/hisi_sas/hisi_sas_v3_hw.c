...
#define FIS_CMD_IS_UNCONSTRAINT(fis) \
...
static void init_reg_v3_hw(struct hisi_hba *hisi_hba)
{
	...
	for (i = 0; i < hisi_hba->n_phy; i++) {
		...
		if (!sas_phy->phy || (sas_phy->phy->maximum_linkrate <
			...
			hisi_sas_get_prog_phy_link_rate(max,
					&prog_phy_link_rate);
		}
			  ...
			  struct hisi_sas_slot *slot)
{
	...
	if (FIS_CMD_IS_UNCONSTRAINT(task->ata_task.fis))
...
static irqreturn_t phy_bcast_v3_hw(int phy_no, struct hisi_hba *hisi_hba)
{
	...
	if ((bcast_status & RX_BCAST_CHG_MSK) &&
		!test_bit(HISI_SAS_FLUTTER_BIT, &hisi_hba->flags))
		...
		struct sas_phy_linkrates *r)
{
	...
	hisi_sas_get_prog_phy_link_rate(max, &prog_phy_link_rate);
	disable_phy_v3_hw(hisi_hba, phy_no);
	msleep(100);
