...
struct domain_device *sas_find_dev_by_rphy(struct sas_rphy *rphy)
{
	...
	//	pr_err("%s3 rphy=%pS i=%d port=%pS shost=%pS\n", __func__, rphy, i, port, shost);
		...
		list_for_each_entry(dev, &port->dev_list, dev_list_node) {
			if (rphy == dev->rphy) {
				pr_err("%s4 dev_list rphy=%pS i=%d port=%pS shost=%pS found_dev=%pS dev->rphy=%pS\n", __func__, rphy, i, port, shost, found_dev, dev->rphy);
				found_dev = dev;
		...
		list_for_each_entry(dev, &port->disco_list, disco_list_node) {
				pr_err("%s4 rphy=%pS i=%d port=%pS shost=%pS found_dev=%pS dev->rphy=%pS\n", __func__, rphy, i, port, shost, found_dev, dev->rphy);
			if (rphy == dev->rphy) {

				pr_err("%s5 disco_list rphy=%pS i=%d port=%pS shost=%pS found_dev=%pS dev->rphy=%pS\n", __func__, rphy, i, port, shost, found_dev, dev->rphy);
				found_dev = dev;
				spin_unlock(&port->dev_list_lock);
				goto found;
			}
...
int sas_target_alloc(struct scsi_target *starget)
{
	...
	pr_err("%s1.0 starget=%pS starget->dev.parent=%pS\n", __func__, starget, starget->dev.parent);
	dev_err(dev, "%s1.1 starget=%pS starget->dev.parent=%pS\n", __func__, starget, starget->dev.parent);
	dev_err(dev->parent, "%s1.2 starget=%pS starget->dev.parent=%pS\n", __func__, starget, starget->dev.parent);

	rphy = dev_to_rphy(starget->dev.parent);
	pr_err("%s2 starget=%pS rphy=%pS\n", __func__, starget, rphy);
	found_dev = sas_find_dev_by_rphy(rphy);
	pr_err("%s3 starget=%pS rphy=%pS found_dev=%pS\n", __func__, starget, rphy, found_dev);
