...
void sas_probe_sata(struct asd_sas_port *port)
{
	...
	list_for_each_entry(dev, &port->disco_list, disco_list_node) {
		...
		pr_err("%s2 port=%pS dev=%pS ap=%pS ata_dev=%pS rphy=%pS parent=%pS %s sdev=%pS devno=%d sdev=%pS\n", 
			__func__, port, dev, dev->sata_dev.ap, ata_dev, rphy, parent, parent ? dev_name(parent) : "", ata_dev->sdev, ata_dev->devno, ata_dev->sdev);
