...
static void sas_ata_task_done(struct sas_task *task)
{
	...
	if (stat->stat == SAS_PROTO_RESPONSE ||
		...
		if (ac) {
			...
			if (!link->sactive) {
				qc->err_mask = ac;
				pr_warn("%s2.1: SAS error 0x%x link->sactive=0x%x task=%pS qc=%pS err_mask=0x%x\n",
					__func__, stat->stat, link->sactive, task, qc, qc->err_mask);
			} else {
...
void sas_ata_std_error_handler(struct ata_port *ap)
{
	struct ata_link *link = &ap->link;
...
static struct ata_port_operations sas_sata_ops = {
	...
	.error_handler		= sas_ata_std_error_handler,
