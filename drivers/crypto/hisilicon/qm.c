...
static int qm_start_qp(struct hisi_qp *qp, unsigned long arg)
{
	...
	dev_dbg(dev, "QM qn=%u, sq_va=%p, sq_dma=%pad, sq_size=%x, cq_va=%p, "
		     "cq_dma=%pad, cq_size=%lx\n",
...
struct hisi_qp *hisi_qm_create_qp(struct hisi_qm *qm, u8 alg_type)
{
	...
	if (qm->uacce_mode == UACCE_MODE_NOUACCE) {
		...
		dev_dbg(dev, "allocate qp dma buf(va=%p, dma=%pad, size=%x)\n",
			qp->scqe.addr, &qp->scqe.dma, qp->scqe.size);
	}
...
int hisi_qm_start_qp(struct hisi_qp *qp, unsigned long arg)
{
	...
	if (!qm->ops->qm_start_qp)
		return -EPERM;

	if (!qp->scqe.dma) {
		dev_err(&qm->pdev->dev, "cannot get qm dma buffer\n");
		return -EINVAL;
	}
			      ...
			      struct uacce_qfile_region *qfr)
{
	...
	case UACCE_QFRT_DUS:
		if (qm->uacce_mode == UACCE_MODE_NOIOMMU) {
			if (sz != qp->scqe.size) {
				dev_warn(dev, "wrong queue size %ld vs %d\n",
					 sz, qp->scqe.size);
				return -EINVAL;
			}
			return remap_pfn_range(vma, vma->vm_start,
				qp->scqe.dma >> PAGE_SHIFT, sz,
				vma->vm_page_prot);
		}
		return -EINVAL;
...
static int hisi_qm_uacce_mem_init(struct hisi_qm *qm,
				  struct qm_dma_buffer *buf)
{
	size_t off = 0;

	qm->eqe.addr = buf->addr;
	qm->eqe.dma = buf->dma;
	qm->eqe.size = sizeof(struct qm_eqe) * QM_Q_DEPTH;
	off += qm->eqe.size;

	qm->sqc.addr = buf->addr + off;
	qm->sqc.dma = buf->dma + off;
	qm->sqc.size = sizeof(struct qm_sqc) * qm->qp_num;
	off += qm->sqc.size;

	qm->cqc.addr = buf->addr + off;
	qm->cqc.dma = buf->dma + off;
	qm->cqc.size = sizeof(struct qm_cqc) * qm->qp_num;
	off += qm->cqc.size;

	qm->aeqe.addr = buf->addr + off;
	qm->aeqe.dma = buf->dma + off;
	qm->aeqe.size = sizeof(struct qm_aeqe) * QM_Q_DEPTH;
	off += qm->aeqe.size;

	qm->eqc.addr = buf->addr + off;
	qm->eqc.dma = buf->dma + off;
	qm->eqc.size = sizeof(struct qm_eqc);
	off += qm->eqc.size;

	qm->aeqc.addr = buf->addr + off;
	qm->aeqc.dma = buf->dma + off;
	qm->aeqc.size = sizeof(struct qm_aeqc);
	off += qm->aeqc.size;

	dev_dbg(&qm->pdev->dev, "kernel-only buffer used (0x%lx/0x%x)\n",
		off, QM_DKO_PAGE_NR << PAGE_SHIFT);
	if (off > (QM_DKO_PAGE_NR << PAGE_SHIFT)) {
		dev_err(&qm->pdev->dev, "kernel-only buffer is too small!\n");
		return -EINVAL;
	}
...
static void hisi_qm_uacce_qp_mem_init(struct hisi_qp *qp,
				      struct qm_dma_buffer *buf)
{
	qp->scqe.addr = buf->addr;
	qp->scqe.dma = buf->dma;
	qp->scqe.size = buf->size;
}
...
static int hisi_qm_uacce_start_queue(struct uacce_queue *q)
{
	...
	if (qm->uacce_mode == UACCE_MODE_UACCE) {
		buf.dma = q->qfrs[UACCE_QFRT_DKO]->iova;
		buf.addr = q->qfrs[UACCE_QFRT_DKO]->kaddr;
		buf.size = q->qfrs[UACCE_QFRT_DKO]->nr_pages >> PAGE_SHIFT;

		ret = hisi_qm_uacce_mem_init(qm, &buf);
		if (ret)
			return ret;
		...
		buf.dma = q->qfrs[UACCE_QFRT_DUS]->iova;
		buf.addr = q->qfrs[UACCE_QFRT_DUS]->kaddr;
		buf.size = q->qfrs[UACCE_QFRT_DUS]->nr_pages >> PAGE_SHIFT;

		hisi_qm_uacce_qp_mem_init(qp, &buf);
	}
			     ...
			     struct uacce_qfile_region *qfr)
{
	...
	if (qfr->type == UACCE_QFRT_DUS) {
		...
		qp->scqe.dma = page_to_phys(qfr->cont_pages);
		qp->scqe.addr = qfr->kaddr; /* it can be 0 */
		qp->scqe.size = qfr->nr_pages >> PAGE_SHIFT;

		dev_dbg(dev, "hisi_qm_uacce_map dus dma=0x%lx, %d pages\n",
			(unsigned long)qp->scqe.dma, qfr->nr_pages);
...
int hisi_qm_init(struct hisi_qm *qm)
{
	...
	if (qm->uacce_mode) {
#ifdef CONFIG_CRYPTO_QM_UACCE
		dev_info(dev, "qm register to uacce mode %d\n", qm->uacce_mode);
		ret = qm_register_uacce(qm);
		if (ret < 0) {
			dev_err(dev, "fail to register uacce\n");
	...
	dev_dbg(dev, "init qm %s to uacce mode %d\n",
		qm->fun_type == QM_HW_PF ? "pf" : "vf", qm->uacce_mode);
