...
{
	union sec_q_aruser_cfg sec_queue_ar_cfg;
...
{
	union sec_q_awuser_cfg sec_queue_aw_cfg;
	void *base;
...
static int hisi_sec_queue_ar_alloc_set(struct wd_queue *queue, u32 alloc)
{
	...
	sec_queue_ar_cfg.u32 = readl_relaxed(base + HISI_SEC_Q_ARUSER_CFG_REG);
	if (alloc == U_SEC_QUEUE_AR_FROCE_ALLOC) {
...
static int hisi_sec_queue_aw_alloc_set(struct wd_queue *queue, u32 alloc)
{
	...
	sec_queue_aw_cfg.u32 = readl_relaxed(base + HISI_SEC_Q_AWUSER_CFG_REG);
	if (alloc == U_SEC_QUEUE_AW_FROCE_ALLOC) {
...
static void hisi_sec_queue_irq_disable(struct wd_queue *queue)
{
	hisi_sec_queue_proc_int_mask_set(queue, 0xffffffff);
}

static void hisi_sec_queue_irq_enable(struct wd_queue *queue)
{
	hisi_sec_queue_proc_int_mask_set(queue, 0);
}

static void hisi_sec_queue_abn_irq_disable(struct wd_queue *queue)
{
	hisi_sec_queue_abnrml_int_mask_set(queue, 0xffffffff);
}

static void hisi_sec_queue_stop(struct wd_queue *queue)
{
	disable_irq(((struct sec_queue_info *)queue->priv)->task_irq);
	hisi_sec_queue_irq_disable(queue);
	hisi_sec_queue_enable(queue, 0x0);
}

static void hisi_sec_queue_start(struct wd_queue *queue)
{
	hisi_sec_queue_irq_enable(queue);
	enable_irq(((struct sec_queue_info *)queue->priv)->task_irq);
	hisi_sec_queue_init_set(queue, 0x03);
...
int hisi_sec_poll(void *queue, int flag)
{
	...
	struct sec_queue_ring *cq_ring = &info->ring[SEC_OUTORDER_RING];
	struct sec_bd_info *msg;
	struct sec_out_bd_info *outorder_msg;
	int msg_index, num = 0;
	void *base = Q_IO_ADDR(sec_queue, 0);

	cq_ring->write = readl(base + HISI_SEC_Q_OUTORDER_WR_PTR_REG);
	cq_ring->read = readl(base + HISI_SEC_Q_OUTORDER_RD_PTR_REG);
	if ((cq_ring->write == cq_ring->read) &&
		(atomic_read(&msg_ring->used) == 0))
		return 0;

	outorder_msg = (struct sec_out_bd_info *)cq_ring->base + cq_ring->read;
	msg_index = outorder_msg->q_id;
	msg = (struct sec_bd_info *)(msg_ring->base) + msg_index;
	while (msg->done) {
		msg_ring->callback(msg);
		msg->done = 0;
		cq_ring->read = (cq_ring->read + 1) % cq_ring->depth;
		writel(cq_ring->read, base + HISI_SEC_Q_OUTORDER_RD_PTR_REG);
		num++;
...
struct wd_queue *hisi_alloc_crypto_queue(struct wd_dev *sec)
{
	...
	ret = _alloc_queue(sec, NULL, &sec_queue);
	if (ret) {
	...
	hisi_sec_queue_start(sec_queue);
...
int hisi_sec_queue_send(void *queue, void *msg)
{
	...
	msg_ring->read = readl(base + HISI_SEC_Q_RD_PTR_REG);
	msg_ring->write = readl(base + HISI_SEC_Q_WR_PTR_REG);
	...
	/*mb here*/
	wmb();
	writel(msg_ring->write, base + HISI_SEC_Q_WR_PTR_REG);
...
static int hisi_sec_queue_res_cfg(struct wd_queue *queue)
{
	...
	for (i = 0; i < HISI_SEC_HW_RING_NUM; i++) {
		...
		page_list = alloc_pages_node(node, GFP_KERNEL, order);
		if (IS_ERR(page_list)) {
