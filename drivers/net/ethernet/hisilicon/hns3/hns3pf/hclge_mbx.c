 ...
 * receive a mailbox message from VF.
 * NOTE:if VF need PF generate a confirm after VF send a mailbox to PF,
 * VF side's calling context can not be atomic,otherwise it may not receive
 * the response message as its expected and timeout will happen at VF side.
 * @vport: pointer to struct hclge_vport
 * @vf_to_pf_req: pointer to hclge_mbx_vf_to_pf_cmd of the original mailbox message
				...
				u8 *resp_data, u16 resp_data_len)
{
	...
	if (resp_data_len > HCLGE_MBX_MAX_RESP_DATA_SIZE) {
		dev_err(&hdev->pdev->dev,
			"PF mbx response fail, len %d exceeds max len %d.\n",
			resp_data_len,
			HCLGE_MBX_MAX_RESP_DATA_SIZE);
	}
	hclge_cmd_setup_basic_desc(&desc, HCLGEVF_OPC_MBX_PF_TO_VF, false);
	resp_pf_to_vf->dest_vfid =
			cpu_to_le16(vf_to_pf_req->mbx_src_vfid);
	...
	if (status)
		dev_err(&hdev->pdev->dev,
			"PF send response to VF fail, status =%d.\n",
			status);
			      ...
			      u16 mbx_opcode, u16 dest_vfid)
{
	...
	if (status)
		dev_err(&hdev->pdev->dev,
			"PF failed to send mbx message to VF, status =%d.\n",
			status);
...
/* HCLGE_MBX_MAP_RING_TO_VECTOR mailbox message:
 * msg[0]: opcode
 * msg[1]:
 * msg[2]: ring_num
 * msg[3]: first ring type
 * msg[4]: first tqp id
...
#define HCLGE_RING_MAP_MBX_BASIC_MSG_NUM	3
	...
	hnae_set_bit(ring_chain->flag, HNAE3_RING_TYPE_B,
		     req->msg[3]);
				       ...
				       struct hclge_mbx_vf_to_pf_cmd *req)
{
	...
	ret = hclge_get_ring_chain_from_mbx(req, &ring_chain,
					    vport);
	if (ret)
		return ret;
				    ...
				    bool gen_resp)
{
	...
	if (mbx_req->msg[1] == HCLGE_MBX_MAC_VLAN_UC_MODIFY) {
		...
		dev_err(&hdev->pdev->dev,
			"Set UC mac address fail, unkonwn cmd=%d.\n",
			mbx_req->msg[1]);
		return -EIO;
	}
	if (gen_resp)
				    ...
				    bool gen_resp)
{
	...
	if (mbx_req->msg[1] == HCLGE_MBX_MAC_VLAN_MC_ADD) {
	...
	} else if (mbx_req->msg[1] == HCLGE_MAC_VLAN_MC_REMOVE) {
		...
		dev_err(&hdev->pdev->dev,
			"Set MC mac address fail, unkonwn cmd=%d.\n",
			mbx_req->msg[1]);
		return -EIO;
	}
	if (gen_resp)
				 ...
				 bool gen_resp)
{
	...
	} else if (mbx_req->msg[1] == HCLGE_MBX_VLAN_TX_OFF_CFG) {
		memcpy(&vport->txvlan_cfg, &mbx_req->msg[2],
		       sizeof(vport->txvlan_cfg));
		status = hclge_set_vlan_tx_offload_cfg(vport);
	} else if (mbx_req->msg[1] == HCLGE_MBX_VLAN_RX_OFF_CFG) {
		memcpy(&vport->rxvlan_cfg, &mbx_req->msg[2],
		       sizeof(vport->rxvlan_cfg));
		status = hclge_set_vlan_rx_offload_cfg(vport);
	}
				       ...
				       bool gen_resp)
{
	...
	resp_data = hdev->hw_tc_map;
	return hclge_gen_resp_to_vf(vport, mbx_req, 0, &resp_data, sizeof(u8));
}
...
void hclge_mbx_handler(struct hclge_dev *hdev)
{
	...
	while (hnae_get_bit(crq->desc[crq->next_to_use].flag,
		...
		default:
			dev_err(&hdev->pdev->dev,
				"Unsupport mail box message, code = %d\n",
				req->msg[0]);
			break;
		}
