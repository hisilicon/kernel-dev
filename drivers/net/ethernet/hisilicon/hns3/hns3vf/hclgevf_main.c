...
static int hclgevf_tqps_update_stats(struct hnae3_handle *handle)
{
	...
	for (i = 0; i < hdev->num_tqps; i++) {
		...
		hclgevf_cmd_setup_basic_desc(&desc[0],
...
static int hclgevf_tqps_get_sset_count(struct hnae3_handle *handle,
				       int stringset)
{
	struct hclgevf_dev *hdev = hclgevf_ae_get_hdev(handle);
...
static int hclgevf_get_sset_count(struct hnae3_handle *handle, int stringset)
{
	int count = 0;
	...
	if (stringset == ETH_SS_TEST)
		...
		count = hclgevf_tqps_get_sset_count(handle, stringset);

	return count;
}

static void hclgevf_get_strings(struct hnae3_handle *handle,
...
#define HCLGEVF_TQPS_RSS_INFO_LEN	8
	u8 resp_msg[HCLGEVF_TQPS_RSS_INFO_LEN] = {0};
	int status;
...
void hclgevf_update_link_status(struct hclgevf_dev *hdev, int link_state)
{
	...
	struct hnae3_client *client = handle->client;
...
static int hclgevf_get_rss_hw_cfg(struct hnae3_handle *handle,
				  u8 *hash, u8 *key)
{
	struct hclgevf_dev *hdev = hclgevf_ae_get_hdev(handle);
	...
	for (key_offset = 0; key_offset < lkup_times; key_offset++) {
		...
		if (status) {
			dev_err(&hdev->pdev->dev,
				"Get hardware RSS config fail, status = %d\n",
				status);
			return status;
		}
		if (key_offset == 2)
...
static int hclgevf_get_rss(struct hnae3_handle *handle, u32 *indir,
			   u8 *key, u8 *hfunc)
{
	struct hclgevf_dev *hdev = hclgevf_ae_get_hdev(handle);
...
int hclgevf_cmd_set_promisc_mode(struct hclgevf_dev *hdev, u32 en)
{
	struct hclge_mbx_vf_to_pf_cmd *req;
	struct hclgevf_desc desc;
	int status;
...
static int hclgevf_set_vlan_tx_off_cfg(struct hnae3_handle *handle,
				       struct hclgevf_tx_vtag_cfg *txvlan_cfg)
{
#define HCLGEVF_VLAN_TX_OFF_MSG_LEN 8
	struct hclgevf_dev *hdev = hclgevf_ae_get_hdev(handle);
	u8 msg_data[HCLGEVF_VLAN_TX_OFF_MSG_LEN];

	memcpy(&msg_data[0], txvlan_cfg, sizeof(*txvlan_cfg));
	return hclgevf_send_mbx_msg(hdev, HCLGE_MBX_SET_VLAN,
				    HCLGE_MBX_VLAN_TX_OFF_CFG, msg_data,
				    HCLGEVF_VLAN_TX_OFF_MSG_LEN, false, NULL,
				    0);
}

static int hclgevf_set_vlan_rx_off_cfg(struct hnae3_handle *handle,
				       struct hclgevf_rx_vtag_cfg *rxvlan_cfg)
{
#define HCLGEVF_VLAN_RX_OFF_MSG_LEN 4
	struct hclgevf_dev *hdev = hclgevf_ae_get_hdev(handle);
	u8 msg_data[HCLGEVF_VLAN_RX_OFF_MSG_LEN];

	memcpy(&msg_data[0], rxvlan_cfg, sizeof(*rxvlan_cfg));
	return hclgevf_send_mbx_msg(hdev, HCLGE_MBX_SET_VLAN,
				    HCLGE_MBX_VLAN_RX_OFF_CFG, msg_data,
				    HCLGEVF_VLAN_RX_OFF_MSG_LEN, false, NULL,
				    0);
}
...
static int hclgevf_en_hw_strip_rxvtag(struct hnae3_handle *handle, bool enable)
{
	...
	hdev->rxvlan_cfg.strip_tag1_en		= false;
	hdev->rxvlan_cfg.strip_tag2_en		= enable;
	hdev->rxvlan_cfg.vlan1_vlan_prionly	= false;
	hdev->rxvlan_cfg.vlan2_vlan_prionly	= false;

	ret = hclgevf_set_vlan_rx_off_cfg(handle, &hdev->rxvlan_cfg);
	if (ret)
		dev_err(&hdev->pdev->dev,
			"Set VF rx_side vlan offload fail, status = %d.\n",
			ret);
...
static void hclgevf_mailbox_service_task(struct work_struct *work)
{
	struct hclgevf_dev *hdev =
		container_of(work, struct hclgevf_dev, mbx_service_task);
...
{
	struct hclgevf_dev *hdev =
		container_of(work, struct hclgevf_dev, service_task);
...
#define HCLGE_DEF_VLAN_TYPE		0x8100
	...
	hdev->txvlan_cfg.accept_tag		= true;
	hdev->txvlan_cfg.accept_untag		= true;
	hdev->txvlan_cfg.insert_tag1_en		= false;
	hdev->txvlan_cfg.insert_tag2_en		= false;
	hdev->txvlan_cfg.default_tag1		= 0;
	hdev->txvlan_cfg.default_tag2		= 0;
	ret = hclgevf_set_vlan_tx_off_cfg(&hdev->nic, &hdev->txvlan_cfg);
	if (ret) {
		dev_err(&hdev->pdev->dev,
			"Set VF tx_side vlan offload fail, status = %d.\n",
			ret);
		return ret;
	}

	hdev->rxvlan_cfg.strip_tag1_en		= false;
	hdev->rxvlan_cfg.strip_tag2_en		= true;
	hdev->rxvlan_cfg.vlan1_vlan_prionly	= false;
	hdev->rxvlan_cfg.vlan2_vlan_prionly	= false;
	ret = hclgevf_set_vlan_rx_off_cfg(&hdev->nic, &hdev->rxvlan_cfg);
	if (ret)
		dev_err(&hdev->pdev->dev,
			"Set VF rx_side vlan offload fail, status = %d.\n",
			ret);
...
{
	if (hdev->roce_client)
		hdev->roce_client->ops->uninit_instance(&hdev->roce,
							0);
...
static int hclgevf_pci_init(struct hclgevf_dev *hdev)
{
	...
	if (ret) {
		...
		dev_warn(&pdev->dev, "Set mask to 32 bits\n");
	}
	...
	hw = &hdev->hw;
	hw->back = hdev;
	hw->io_base = pci_ioremap_bar(pdev, 2);
	if (!hw->io_base) {
...
static int hclgevf_init_ae_dev(struct hnae3_ae_dev *ae_dev)
{
	...
	if (ret) {
		dev_err(&pdev->dev, "Misc IRQ(vector0) init error, ret = %d.\n",
			ret);
	...
	if (ret) {
		dev_err(&pdev->dev, "Configuration fetch failed(%d)\n", ret);
		goto err_config;
	}
	...
	if (ret) {
		dev_err(&hdev->pdev->dev,
			"set mta filter mode failed ret=%d\n", ret);
		goto err_config;
	}
	...
	if (ret) {
		dev_err(&hdev->pdev->dev,
			"Initialize VLAN cfg failed ret=%d\n", ret);
		goto err_config;
	}

	pr_info("%s driver initialize finished.\n", HCLGEVF_DRIVER_NAME);
...
static void hclgevf_uninit_ae_dev(struct hnae3_ae_dev *ae_dev)
{
	...
	hclgevf_state_uninit(hdev);
	hclgevf_destroy_cmd_queue(&hdev->hw);
	hclgevf_misc_irq_uninit(hdev);
	hclgevf_uninit_msi(hdev);
	hclgevf_pci_uninit(hdev);
	ae_dev->priv = NULL;
