			      ...
			      struct mlx5_core_mkey *mkey)
{
	...
	for_each_sgtable_dma_page(&state->mig_data.table, &dma_iter, 0)
				       ...
				       unsigned long offset)
{
	...
	if (offset < data->last_offset || !data->last_offset_sg) {
		data->last_offset = 0;
		data->last_offset_sg = data->table.sgl;
	...
	for_each_sg(data->last_offset_sg, sg,
			data->table.orig_nents - data->sg_last_entry, i) {
...
void mlx5vf_free_migration_data(struct migration_data *data)
{
	...
	for_each_sgtable_page(&data->table, &sg_iter, 0)
		__free_page(sg_page_iter_page(&sg_iter));

	sg_free_table(&data->table);
}
			       ...
			       unsigned int npages)
{
	...
	page_list = kcalloc(to_fill, sizeof(*page_list), GFP_KERNEL);
	if (!page_list)
		return -ENOMEM;
	...
	do {
		...
		to_alloc -= filled;
		sg_end = __sg_alloc_table_from_pages(&data->table, page_list,
						     filled,
						     0, filled << PAGE_SHIFT,
						     UINT_MAX,
						     data->sg_end,
...
{
	mlx5vf_free_migration_data(&state->mig_data);
			       ...
			       struct mlx5_vhca_state_data *state)
{
	...
	err = dma_map_sgtable(mdev->device, &state->mig_data.table,
			      DMA_FROM_DEVICE, 0);
	if (err)
		goto err_reg_dma;
	...
	dma_unmap_sgtable(mdev->device, &state->mig_data.table,
			  DMA_FROM_DEVICE, 0);
	mlx5_put_core_dev(mdev);
			       ...
			       struct mlx5_vhca_state_data *state)
{
	...
	/* fallthrough to clean up resources */
	mlx5_core_destroy_mkey(mdev, &mkey);
err_mkey:
	dma_unmap_sgtable(mdev->device, &state->mig_data.table,
			  DMA_TO_DEVICE, 0);
