...
static int vf_qm_state_resume(struct hisi_acc_vf_mig_info *vmig)
{
	struct device *dev = &vmig->vf_dev->dev;
	struct hisi_qm *vf_qm = &vmig->vf_qm;
	int ret;
					...
					u32 state)
{
	...
	case VFIO_DEVICE_STATE_RUNNING:
		if (vmig->device_state == VFIO_DEVICE_STATE_RESUMING)
			ret = vf_qm_state_resume(vmig);
		break;
	case VFIO_DEVICE_STATE_SAVING | VFIO_DEVICE_STATE_RUNNING:
		ret = pf_qm_state_pre_save(vmig);
		break;
	case VFIO_DEVICE_STATE_SAVING:
		ret = vf_qm_state_save(vmig);
		break;
...
static int hisi_acc_vf_data_transfer(struct hisi_acc_vf_mig_info *vmig,
				    ...
				    bool iswrite)
{
	...
	if (pos >= HISI_ACC_MIG_REGION_DATA_OFFSET)
		return hisi_acc_vf_data_transfer(vmig, buf,
			count, pos - HISI_ACC_MIG_REGION_DATA_OFFSET,
			iswrite);

	switch (pos) {
	case VDM_OFFSET(device_state):
		if (count != sizeof(vmig->device_state))
			return -EFAULT;
		...
		if (iswrite) {
			...
			ret = hisi_acc_vf_set_device_state(vmig, device_state);
			if (ret)
				return ret;
		} else {
			ret = copy_to_user(buf, &vmig->device_state, count);
			if (ret)
				return -EFAULT;
		}
	...
	case VDM_OFFSET(data_size):
		if (count != sizeof(vmig->data_size))
			return -EINVAL;

		if (iswrite) {
			ret = copy_from_user(&vmig->data_size, buf, count);
			if (ret)
				return -EFAULT;
			...
			ret = copy_to_user(buf, &vmig->data_size, count);
			if (ret)
				return -EFAULT;
		}
...
static int hisi_acc_vfio_pci_init(struct hisi_acc_vf_core_device *hisi_acc_vdev)
{
	...
	struct pci_dev *pf_dev = vf_dev->physfn;
	struct hisi_qm *vf_qm = &vmig->vf_qm;
	 ...
	 * which doesn't have support for PCIe P2P.
	 */
	vf_qm->io_base =
...
{
	struct hisi_acc_vf_core_device *his_acc_vdev;
	int ret;

	his_acc_vdev = kzalloc(sizeof(*his_acc_vdev), GFP_KERNEL);
	if (!his_acc_vdev)
		return -ENOMEM;

	vfio_pci_core_init_device(&his_acc_vdev->core_device, pdev,
				  &hisi_acc_vfio_pci_ops);

	ret = vfio_pci_core_register_device(&his_acc_vdev->core_device);
	if (ret)
		goto out_free;

	dev_set_drvdata(&pdev->dev, his_acc_vdev);
	return 0;

out_free:
	vfio_pci_core_uninit_device(&his_acc_vdev->core_device);
	kfree(his_acc_vdev);
	return ret;
}
