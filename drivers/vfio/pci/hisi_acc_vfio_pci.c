...
static int vf_qm_state_stop(struct acc_vf_migration *acc_vf_dev, u32 state)
{
	struct acc_vf_data *vf_data = acc_vf_dev->vf_data;
	struct hisi_qm *vf_qm = acc_vf_dev->vf_qm;
	int ret;

	/* Check if we need to reset VF */
	if (vf_data->vf_state == VF_NOT_RESET_READY)
		return 0;
					...
					u32 state)
{
	...
	if (!vfio_change_migration_state_allowed(state, mig_ctl->device_state))
		return -EINVAL;
	...
	case VFIO_DEVICE_STATE_STOP:
		ret = vf_qm_state_stop(acc_vf_dev, state);
		break;
