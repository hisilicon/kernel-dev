...
static int vfio_device_fops_open(struct inode *inode, struct file *filep)
{
	 ...
	 */
	core_dev = vfio_allocate_core_device(device, true);
	if (IS_ERR(core_dev)) {
		ret = PTR_ERR(core_dev);
		goto err_put_registration;
	}

	filep->private_data = core_dev;
...
{
	struct vfio_core_device *core_dev = file->private_data;

	if (file->f_op != &vfio_device_fops)
		return NULL;
	return core_dev->device;
}
...
static void vfio_device_file_set_kvm(struct file *file, struct kvm *kvm)
{
	...
	/*
	 * The kvm is first recorded in the core_dev, and will be propagated
