		...
		size_t size, u64 dma_limit, struct device *dev)
{
	...
	if (size > atomic64_read(&iovad->rcache_max))
		atomic64_set(&iovad->rcache_max, size);
...
unsigned long iommu_dma_get_cached_dma_len(struct device *dev)
{
	struct iommu_domain *domain = iommu_get_domain_for_dev(dev);
	struct iommu_dma_cookie *cookie;
	...
	return iova_cache_len();
}
EXPORT_SYMBOL(iommu_dma_get_cached_dma_len);
