...
#define STRTAB_STE_1_S1STALLD		(0UL << 27)
...
static int arm_smmu_device_dt_probe(struct platform_device *pdev)
{
	 ...
	 * 3, device, nGnRE
	 */
	prop = of_get_property(dev->of_node, "smmu-cb-memtype", &len);
	for (i = 0; prop && (i < (len / 4) - 1); i += 2) {
		...
		if (smmu->features & ARM_SMMU_FEAT_2_LVL_STRTAB) {
			ret = arm_smmu_init_l2_strtab(smmu, sid);
			if (ret) {
				dev_err(dev, "Alloc ste failed 0x%llx\n", sid);
				goto out_free_structures;
			}
		}
		 ...
		 */
		ste[0] = (4 << 1) | (1 << 0);
		ste[1] = ((u64)0xbf000 << 32);	/* Data, Priv, NS, ISH */
		ste[2] = 0;
		ste[3] = 0;
...
static int arm_of_iommu_init(struct device_node *np)
{
	of_iommu_set_ops(np, &arm_smmu_ops);

	return 0;
}
IOMMU_OF_DECLARE(arm_smmu_v3, "arm,smmu-v3", arm_of_iommu_init);
