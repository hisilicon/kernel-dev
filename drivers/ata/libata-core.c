			      ...
			      unsigned long timeout)
{
	...
	pr_err("%s2 qc=%pS scmd=%pS rq=%pS sdev=%pS ap=%pS calling blk_execute_rq_nowait\n", __func__, qc, scmd, rq, sdev, ap);
	blk_execute_rq_nowait(rq, true);

	if (ap->ops->error_handler)
		ata_eh_release(ap);

	pr_err("%s3 qc=%pS scmd=%pS rq=%pS sdev=%pS ap=%pS calling wait_for_completion\n", __func__, qc, scmd, rq, sdev, ap);
	rc = 0;
	wait_for_completion(&wait);
	pr_err("%s4 qc=%pS scmd=%pS rq=%pS sdev=%pS ap=%pS got completion RQF_TIMED_OUT=%d\n",
	 __func__, qc, scmd, rq, sdev, ap, !!(rq->rq_flags & RQF_TIMED_OUT));
	...
	*tf = qc->result_tf;
	...
	ata_qc_free(qc);
	pr_err("%s9 qc=%pS link->active_tag=%d link->preempted_tag=%d\n", __func__, qc, link->active_tag, link->preempted_tag);
