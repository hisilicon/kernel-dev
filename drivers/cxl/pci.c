...
static int cxl_mem_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	if (rc != irqs) {
		...
		dev_dbg(&pdev->dev, "No interrupts available for DOE\n");
	} else {
		/*
		 ...
		 */
		pci_set_master(pdev);
		rc = devm_add_action_or_reset(&pdev->dev, cxl_mem_free_irq_vectors,
					      pdev);
		if (rc)
			return rc;
	}
	...
	rc = devm_add_action_or_reset(&pdev->dev, cxl_mem_doe_unregister_all, pdev);
	if (rc)
		return rc;
