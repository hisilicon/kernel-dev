...
static unsigned long asid_contex_pinned_get(struct asid_info *info,
					    atomic64_t *pasid,
					    refcount_t *pinned)
{
	unsigned long flags;
...
static void asid_contex_pinned_put(struct asid_info *info, atomic64_t *pasid,
				   refcount_t *pinned)
{
	unsigned long flags;
	u64 asid = atomic64_read(pasid);
...
unsigned long arm64_mm_context_get(struct mm_struct *mm)
{
	...
	asid = asid_contex_pinned_get(info, &mm->context.id,
				      &mm->context.pinned);
...
void arm64_mm_context_put(struct mm_struct *mm)
{
	...
	asid_contex_pinned_put(info, &mm->context.id, &mm->context.pinned);
}
