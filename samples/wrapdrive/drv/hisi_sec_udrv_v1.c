...
static void hisi_sec_cipher_alg_set(struct sec_bd_info *bd,
				struct sec_wd_msg *wd_msg)
{
	uint32_t sec_alg;
	...
	struct sec_bd_info *sec_msg, struct sec_wd_msg *wd_msg, __u32 index)
{
	...
	if (sec_msg->done == 1) {
		if (sec_msg->bd_invalid == 1)
			status = -2;
		if (sec_msg->icv_or_s_key_en == 3)
			status = -2;
...
static int hisi_sec_vfio_ring_init(struct wd_queue *q)
{
	...
	for (i = 0; i < HISI_SEC_HW_RING_NUM; i++) {
		info->ring[i].depth = HISI_SEC_QUEUE_LEN;
	...
	info->out_ring.depth = HISI_SEC_QUEUE_LEN;
...
int hisi_sec_v1_batch_send(struct wd_queue *q, void **req, int num)
{
	...
	while (num--) {
		...
		sec_msg = (struct sec_bd_info *)msg_ring->base +
			msg_ring->write;
		memset(sec_msg, 0, 64);
...
int hisi_sec_v1_batch_recv(struct wd_queue *q, void **resp, int num)
{
	...
	while (num--) {
		...
		ret = hisi_sec_v1_msg_2_wd_msg(q,
			(struct  sec_bd_info *)msg_ring->base + msg_index,
