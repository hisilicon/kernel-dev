...
struct wd_rsa_ctx {
	...
	int is_pubkey_format;
	int is_privkey1_format;
	int is_privkey2_format;
...
/* len: actual len of key parameter */
/* buf_len: key parameter generated by openssl */
			     ...
			     struct hisi_hpre_sqe *hw_msg)
{
	...
	if (strncmp(alg, "rsa", 3))  {
		HPRE_TST_PRT("alg=%s,rsa algorithm support only now!\n", alg);
		return -1;
	}
	...
	if (rsa_msg->op_type == WD_RSA_SIGN) {
		...
		HPRE_TST_PRT("rsa ALG support only sign and verify now!\n");
		return -1;
	}
			     ...
			     struct hisi_hpre_sqe *hw_msg)
{
	...
	if (strncmp(alg, "dh", 2))  {
		HPRE_TST_PRT("alg=%s,rsa/dh algos support only now!\n", alg);
		return -1;
	}
	...
	if (dh_msg->op_type == WD_DH_PHASE1 ||
	  dh_msg->op_type == WD_DH_PHASE2) {
		if (ctx->is_g2 && dh_msg->op_type == WD_DH_PHASE1) {
			...
			(void)hpre_bn_format(g, ctx->key_size, dh_msg->gbytes);
		...
		(void)hpre_bn_format(x, ctx->key_size, dh_msg->xbytes);

		p = x + ctx->key_size;
		memcpy(p, (void *)dh_msg->p, ctx->key_size);
		(void)hpre_bn_format(p, ctx->key_size, dh_msg->pbytes);
			    ...
			    char *crt_privkey_file, int is_file)
{
	...
	pubkey->ebytes = BN_bn2bin(e, pubkey->e);
	ret = hpre_bn_format(pubkey->e, key_bytes, pubkey->ebytes);
	if (ret) {
		HPRE_TST_PRT("hpre_bn_format pubkey_e bin fail!\n");
		goto gen_fail;
	}
	pubkey->nbytes = BN_bn2bin(n, pubkey->n);
	ret = hpre_bn_format(pubkey->n, key_bytes, pubkey->nbytes);
	if (ret) {
		HPRE_TST_PRT("hpre_bn_format pubkey_n bin fail!\n");
		goto gen_fail;
	}
	...
	prikey->pkey2.dpbytes = BN_bn2bin(dmp1, prikey->pkey2.dp);
	ret = hpre_bn_format(prikey->pkey2.dp, key_bytes / 2, prikey->pkey2.dpbytes);
	if (ret) {
		HPRE_TST_PRT("hpre_bn_format prikey->pkey2.dp bin fail!\n");
		goto gen_fail;
	}
	prikey->pkey2.dqbytes = BN_bn2bin(dmq1, prikey->pkey2.dq);
	ret = hpre_bn_format(prikey->pkey2.dq, key_bytes / 2, prikey->pkey2.dqbytes);
	if (ret) {
		HPRE_TST_PRT("hpre_bn_format prikey->pkey2.dq bin fail!\n");
		goto gen_fail;
	}
	prikey->pkey2.pbytes = BN_bn2bin(p, prikey->pkey2.p);
	ret = hpre_bn_format(prikey->pkey2.p, key_bytes / 2, prikey->pkey2.pbytes);
	if (ret) {
		HPRE_TST_PRT("hpre_bn_format prikey->pkey2.p bin fail!\n");
		goto gen_fail;
	}
	prikey->pkey2.qbytes = BN_bn2bin(q, prikey->pkey2.q);
	ret = hpre_bn_format(prikey->pkey2.q, key_bytes / 2, prikey->pkey2.qbytes);
	if (ret) {
		HPRE_TST_PRT("hpre_bn_format prikey->pkey2.q bin fail!\n");
		goto gen_fail;
	}
	prikey->pkey2.qinvbytes = BN_bn2bin(iqmp, prikey->pkey2.qinv);
	ret = hpre_bn_format(prikey->pkey2.qinv, key_bytes / 2, prikey->pkey2.qinvbytes);
	if (ret) {
		HPRE_TST_PRT("hpre_bn_format  bin fail!\n");
		goto gen_fail;
	}
	...
	ret = hpre_bn_format(prikey->pkey1.d, key_bytes, prikey->pkey1.dbytes);
	if (ret) {
		HPRE_TST_PRT("hpre_bn_format prikey->pkey1.d bin fail!\n");
		goto gen_fail;
	}
	prikey->pkey1.nbytes = BN_bn2bin(n, prikey->pkey1.n);
	ret = hpre_bn_format(prikey->pkey1.n, key_bytes, prikey->pkey1.nbytes);
	if (ret) {
		HPRE_TST_PRT("hpre_bn_format prikey->pkey1.n bin fail!\n");
		goto gen_fail;
	}
				...
				unsigned int block_size, unsigned int block_num)
{
	...
	if (block_size > 4096) {
		HPRE_TST_PRT("current blk size is bellow 4k\n");
		return NULL;
	}
	rsv_mm_sz = block_size * block_num;
	if (rsv_mm_sz > 0x400000) {
		HPRE_TST_PRT("current mem size should be bellow 4M.\n");
		return NULL;
	}
	...
	if (!pool) {
		HPRE_TST_PRT("Alloc pool handle fail!\n");
		return NULL;
	}
			...
			struct wd_rsa_op_data *opdata)
{
	...
	if (!out) {
		free(in);
		HPRE_TST_PRT("%s:malloc crt out fail!\n", __func__);
		return -ENOMEM;
	}
	...
	if (opdata->op_type == WD_RSA_GENKEY) {
		...
		if (ctx->is_crt) {
			...
			if (memcmp(qinv, out + offset, ctx->key_size / 2)) {
				HPRE_TST_PRT("keygen  N  mismatch!\n");
				return -EINVAL;
			}
		...
		if (key && ctx->is_crt) {
			...
			if (ret <= 0) {
				HPRE_TST_PRT("d set0_crt_params err!\n");
				return -EINVAL;
			}
			ret = RSA_set0_factors(hpre_test_rsa, p, q);
			if (ret <= 0) {
				HPRE_TST_PRT("d set0_factors err!\n");
				return -EINVAL;
			}
			...
			ret = RSA_set0_key(hpre_test_rsa, nn, e, NULL);
			if (ret <= 0) {
				HPRE_TST_PRT("e set0_key crt err!\n");
				return -EINVAL;
			}
...
#define DH_GENERATOR_5          5
	...
	if (!ag_bin) {
		HPRE_TST_PRT("malloc paras fail!\n");
		ret = -ENOMEM;
		goto dh_err;
	}
	...
	opdata_a.gbytes = BN_bn2bin(ag, ag_bin);
	...
	if (openssl_check) {
		...
		ret = hpre_bn_format(apub_key_bin, ctx->key_size, ctx->key_size);
		if (ret) {
	...
	if (openssl_check) {
		...
		ret = hpre_bn_format(bpub_key_bin, ctx->key_size, ctx->key_size);
		if (ret) {
			HPRE_TST_PRT("hpre_bn_format bpub_key bin fail!\n");
			goto dh_err;
		}
	...
	if (openssl_check) {
		ret = hpre_bn_format(abuf, ctx->key_size, ctx->key_size);
		if (ret) {
			HPRE_TST_PRT("hpre_bn_format bpub_key bin fail!\n");
			goto dh_err;
		}
...
void  *_hpre_sys_test_thread(void *data)
{
	...
	float time, speed;
	enum alg_op_type op_type;
	int thread_num, log_intval[LOG_INTVL_NUM] = {0x1ff, 0x3ff, 0x7ff,
...
request_queue:
	...
	while (1) {
		...
		if (intval_index <= 0)
			intval_index = 0;
		if (intval_index > LOG_INTVL_NUM - 1)
			intval_index = LOG_INTVL_NUM - 1;
		if (with_log && !(i & log_intval[intval_index])) {
